...
`ROADMAP.md` Phase 4 is the current phase. The job queue (`@seo/queue`), the audit scheduler (`@seo/scheduler`), the grader (`@seo/grader`) and durable queue storage (`@seo/job-store`) are in; lease expiry (@seo/job-store, @seo/queue) is in, so a second worker can share a queue namespace, and lanes hold across workers, so two of them never crawl one host together; what remains is detector coverage — 80 of v5.0's 134 detectors are unimplemented, which is the single thing most limiting what an audit can say — and the v5.0 workbook's second assessment (READY FOR CUTOVER, evidence classes, review freshness). Phases 5-8 cover rendered crawl, external body storage, the audit API, and the dashboard.
